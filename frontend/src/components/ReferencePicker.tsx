...
export default function ReferencePicker({ sessionId, journal, onConfirm, onCancel }: Props) {
  ...
  const statusIcon = (status: string) => {
    ...
    if (status === 'pdf_downloaded') return { color: 'text-blue-400', label: 'PDF' };
  ...
  return (
    <div className="bg-bg-card rounded-lg border border-slate-700 overflow-hidden">
      ...
      <div className="px-5 py-3 border-b border-slate-700 flex items-center gap-2 flex-wrap">
        <button
          onClick={selectRecommended}
          className="text-xs px-3 py-1.5 bg-accent/10 text-accent rounded-md hover:bg-accent/20 transition-colors"
        >
          Select Recommended
        </button>
        <button
        ...
        <button
          onClick={selectAll}
          className="text-xs px-3 py-1.5 bg-slate-600/30 text-text-secondary rounded-md hover:bg-slate-600/50 transition-colors"
        >
          Select All
        </button>
        <button
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      {p.recommended && (
                        <span className="flex-shrink-0 text-[10px] bg-accent/15 text-accent px-1.5 py-0.5 rounded">rec</span>
                      )}
                    </div>
